...
function Custom() {
  ...
  const selectTemplate = (event) => {
    const templateStats = templates[event.target.value].map((v) => {
      return { ...v, value: '' };
    });
  ...
  const handleInputChange = (event) => {
    ...
    tempStats[event.target.getAttribute('index')][event.target.name] =
      event.target.value;
    setStats(tempStats);
  };
  ...
  return (
    <div className='custom'>
      <h1 className='custom-header'>Radar Generator</h1>
            ...
            {stats.map((s, i) => (
              <div key={i} className='input-row'>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                ></input>
                <span index={i} className='delete' onClick={deleteRow}>
                  x
                </span>
              </div>
            ))}
