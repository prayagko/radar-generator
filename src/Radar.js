  ...
  const chartAnnotations = [0, 50, 100, 150, 200, 250]
    let scales = data.map((d,i)=>{
      ...
      const r = {field: d.field,
        ...
        value: d.value?(d3.scaleLinear().domain([d.min, d.max]).range([50,250]))(d.value):0,
      }
      if(r.value<0){
        r.value = 0
      }
    ...
    return (
        <svg width={width+200} height={height+200}>
          <path fill={props.color} d={line(pathCoordinates) } opacity = {0.5}></path>
          {scales.map((i, index) => {
          ...
          {chartAnnotations.map((i) => (
            <g key={i} transform={`translate(${width / 2}, ${height / 2})`}>
              <circle r={i} fill="none" stroke="#999" stroke-alignment="inner" strokeWidth="25" opacity = {0.4}/>
            </g>
          ))}
          </svg>  
      )
